...
/* ─── Room grid ─────────────────────────────────────────────────── */
export function RoomGrid({ onSelect }: { onSelect: (room: Room) => void }) {
  return (
      ...
      {ROOMS.map((room) => (
        <button
          ...
          className={`
            ...
            bg-white/5 backdrop-blur-sm p-5 sm:p-7 text-center
        ...
        >
          ...
          <span className={`relative font-black text-base sm:text-lg leading-tight ${room.text}`}>
            {room.name}
          </span>
        </button>
      ))}
...
export function RegistrationForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (activePlayers.some((p) => !p.name.trim())) {
    ...
    activePlayers.forEach((p, i) => {
      ...
      if (p.universityId.trim()) payload[`player${i + 1}_universityId`] = p.universityId.trim();
    });

    try {
      const id = process.env.NEXT_PUBLIC_FORMSPREE_ID;
      if (!id) throw new Error('Formspree not configured.');
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-6 flex items-center gap-4">
        <button
          onClick={onBack}
          className="text-gray-500 hover:text-white text-sm font-medium transition-colors shrink-0"
        >
          ← Back
        </button>
        <div className={`px-5 py-2 rounded-full bg-gradient-to-r ${room.gradient} text-white font-black text-base shadow-lg`}>
          {room.name}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-3">
          {Array.from({ length: teamSize }).map((_, i) => (
            <div key={i} className={`rounded-2xl border ${room.border} bg-white/5 p-4`}>
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full py-4 rounded-2xl font-black text-base text-white bg-gradient-to-r ${room.gradient} shadow-lg transition-all duration-200 hover:opacity-90 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100`}
        >
...
/* ─── Success screen ─────────────────────────────────────────────── */
export function SuccessScreen({ data, onReset }: { data: SuccessData; onReset: () => void }) {
  const { room, players, timestamp } = data;
  return (
    <div className="max-w-sm mx-auto text-center">
      <div className="text-5xl mb-5">🔐</div>
      <div className={`rounded-3xl border ${room.border} bg-white/5 p-7 mb-5`}>
        ...
        <p className="text-gray-500 text-xs mb-5">Registered at {timestamp}</p>
        <div className="space-y-2 text-left">
          {players.map((p, i) => (
            <div key={i} className="flex items-center gap-3 bg-black/30 rounded-xl px-4 py-3">
              <span className={`text-xs font-black ${room.text} w-6 shrink-0`}>P{i + 1}</span>
              <div>
                <p className="text-white font-semibold text-sm leading-tight">{p.name}</p>
                {p.universityId && <p className="text-gray-500 text-xs">ID: {p.universityId}</p>}
              </div>
            </div>
          ))}
        </div>
...
/* ─── Main embeddable section ────────────────────────────────────── */
export default function EscapeRoomRegistration() {
  ...
  return (
    <section className="bg-gray-950 py-20 px-4 relative overflow-hidden">
      {/* Ambient glows */}
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/4 top-0 w-96 h-96 bg-violet-700/10 blur-3xl rounded-full" />
        <div className="absolute right-1/4 bottom-0 w-96 h-96 bg-cyan-700/10 blur-3xl rounded-full" />
      </div>

      <div className="relative max-w-4xl mx-auto">
        {/* Section header */}
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-violet-400 text-xs font-bold tracking-widest uppercase mb-5">
            <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse" />
            April 1–2, 2026 · On Campus
          </span>
          ...
          <p className="text-gray-400 font-semibold text-sm">
            Player Registration — Select your room to begin
          </p>
        </div>
        ...
        {success ? (
          ...
          <RoomGrid onSelect={setSelectedRoom} />
        )}
